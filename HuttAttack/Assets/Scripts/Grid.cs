...
public class Grid : MonoBehaviour {
	...
	private DragDropSpell pressedSpell;
	...
	public List<GamePiece> GetMatch(GamePiece piece, int newX, int newY)
	{
		if(piece.IsColored())
		{
			...
			// Didn't find anything going horizontal fisrt
	...
	public void ClearFireball()
	{
		...
		{
			for (int i = 0; i < spell.fireballRange.Count; i++)
			{
				if ( (enteredPiece.X + spell.fireballRange[i].x < xDim && enteredPiece.X + spell.fireballRange[i].x >= 0) && 
					(enteredPiece.Y + spell.fireballRange[i].y < yDim && enteredPiece.Y + spell.fireballRange[i].y >= 0) )
				{
					ClearPiece(enteredPiece.X + spell.fireballRange[i].x,enteredPiece.Y + spell.fireballRange[i].y);
				}
			}
			enteredPiece = null;
	...
	public void ClearIce()
	{
		if (enteredPiece != null)
		{
			for (int i = 0; i < spell.iceRange.Count; i++)
			{
				if ( (enteredPiece.X + spell.iceRange[i].x < xDim && enteredPiece.X + spell.iceRange[i].x >= 0) && 
					(enteredPiece.Y + spell.iceRange[i].y < yDim && enteredPiece.Y + spell.iceRange[i].y >= 0) )
				{
					ClearPiece(enteredPiece.X + spell.iceRange[i].x,enteredPiece.Y + spell.iceRange[i].y);
				}
			}
			enteredPiece = null;
	...
	public void ClearLightning()
	{
		...
		if (enteredPiece != null)
		{
			int x = enteredPiece.X;
			int y = enteredPiece.Y;
			Debug.Log("Ice start pos: " + x + "," + y);
			while (y <= yDim)
			{
				...
				// Select next position to clear
				if (left == true)
				{
					x += spell.lightningRange[1].x;
					y += spell.lightningRange[1].y;
				}
				else if (right == true)
				{
					x += spell.lightningRange[0].x;
					y += spell.lightningRange[0].y;
				}
